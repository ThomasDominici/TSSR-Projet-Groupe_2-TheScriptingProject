...
#-------------------- fonctions sur les utilisateurs-------------------
	# Fonction pour ajouter un utilisateur
		ajouter_utilisateur() {
			...
			then
    		echo "utilisateur $utilisateur existe déjà sur la machine distante."
    		echo "Veuillez choisir un autre nom utilisateur."
			else
    		# Créer utilisateur
   			useradd -m "$utilisateur"
    		echo "Utilisateur $utilisateur a été créé avec succès sur la machine distante."
			fi "
			}
	...
	# Fonction pour supprimer un utilisateur
		supprimer_utilisateur() {
   		...
   		ssh $user@$ipAddress "userdel -r $utilisateur"
	...
	# Fonction pour suspendre un utilisateur
		suspendre_utilisateur() {
     	...
     	ssh $user@$ipAddress "usermod -L $utilisateur"
	...
	# Fonction pour modifier un utilisateur
		modifier_utilisateur() {
         	...
         	ssh $user@$ipAddress "usermod -l $nouveau_nom $utilisateur"
	...
	# Fonction pour changer le mot de passe d'un utilisateur
		changer_mot_de_passe() {
     	...
     	ssh $user@$ipAddress "passwd $utilisateur"
	...
	# Fonction pour ajouter un utilisateur à un groupe
       ...
       ssh $user@$ipAddress "usermod -aG $groupe $utilisateur"
	...
	# Fonction pour sortir un utilisateur d'un groupe
		...
		sortir_du_groupe() {
       	...
       	ssh $user@$ipAddress "deluser $utilisateur $groupe"
...
# --------------------fonctions sur les machines-----------------
	...
	# Fonction d'arrêt
		...
		arreter() {
   	 	...
   	 	ssh $user@$ipAddress "shutdown -h now" 
	...
	# Fonction de mise à jour du système
    	...
    	ssh $user@$ipAddress "apt update && apt upgrade -y"
	...
	# Fonction de verrouillage
    	...
    	ssh $user@$ipAddress "systemctl suspend"
	...
	# Fonction de dévérrouillage
    	...
    	ssh $user@$ipAddress "systemctl suspend -i"
	...
	definir_regles_pare_feu() {
    	...
    	ssh $user@$ipAddress "iptables -A INPUT -p tcp --dport 22 -j ACCEPT"

    	ssh $user@$ipAddress "iptables -A INPUT -j DROP"
