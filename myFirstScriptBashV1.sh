...
#InitialisationSSH
function initialisationSSH() 
{
read -p "Sur quel utilisateur de la machine allez vous vous connecter : " utilisateurSSH
read -p "Entrez l'adresse IP de la machine cible: " AdresseIpSSH
if [ -z $utilisateurSSH ] || [ -z $AdresseIpSSH ]
then
	clear
	echo "Veuillez rentrer un nom d'utilisateur ou une adresse IP" 
	initialisationSSH
fi 
}
...
then
	...
	journalisationInitialisation
else
	if [ -d $chemin ]
	then
		echo "Destination prise en compte"
	else
...
#Regroupement des fonctions d'initialisation Journalisation SSH 
function regroupementDeux
{
initialisationJournalisation
initialisationSSH
}

#Regroupement des fonctions d'initialisation Journalisation SSH et utilisateur
function regroupementTrois
{
initialisationJournalisation
initialisationSSH
initialisationUtilisateur
...
#Regroupement des fonctions d'initialisation Journalisation SSH  utilisateur et groupe
function regroupementQuatre
{
initialisationJournalisation
initialisationSSH
initialisationUtilisateur
...
ssh $utilisateurSSH@$AdresseIpSSH 	"
					...
					then
						'$utilisateur existe déjà sur la machine distante.'
						echo 'Veuillez choisir un autre nom utilisateur.'
					else
	   					useradd -m '$utilisateur' && echo '$utilisateur a été créé avec succès sur la machine distante.' 
					fi"
echo " $(date "+%F")_$(date "+%H:%M:%S")_$chemin  Ajout de l'utilisateur $utilisateur" >> /var/log/log_actions.log
}
...
ssh $utilisateurSSH@$AdresseIpSSH	"
					if cat /etc/passwd | grep $utilisateur > /dev/null
					then
						userdel -r $utilisateur && echo '$utilisateur a été supprimé'
					else
						echo '$utilisateur exsiste pas et ne peut être supprimé'
					fi"		
echo " $(date "+%F")_$(date "+%H:%M:%S")_$chemin  Suppression de $utilisateur" >> /var/log/log_actions.log
}
...
ssh $utilisateurSSH@$AdresseIpSSH 	"
					if cat /etc/passwd | grep $utilisateur > /dev/null
					then
						usermod -L $utilisateur && echo '$utilisateur a été suspendu'
					else
						echo '$utilisateur exsiste pas et ne peut être supprimé'
					fi"
...
{
regroupementDeux
ssh $utilisateurSSH@$AdresseIpSSH "shutdown -h now" 
...
{
regroupementDeux
ssh $utilisateurSSH@$AdresseIpSSH "reboot"
...
{
regroupementDeux
ssh $utilisateurSSH@$AdresseIpSSH "apt update && apt upgrade -y"
...
{
regroupementDeux
ssh $utilisateurSSH@$AdresseIpSSH "systemctl suspend"
...
{
regroupementDeux
ssh $utilisateurSSH@$AdresseIpSSH "systemctl suspend -i"
...
#ssh $utilisateurSSH@$AdresseIpSSH "iptables -A INPUT -p tcp --dport 22 -j ACCEPT"
#ssh $utilisateurSSH@$AdresseIpSSH "iptables -A INPUT -j DROP"
...
{
regroupementTrois
ssh $utilisateurSSH@$AdresseIpSSH last $utilisateur | head -n 1 >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt
...
{
regroupementTrois
ssh $utilisateurSSH@$AdresseIpSSH passwd -S $utilisateur | awk '{print $3}' >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt
echo "Résultats enregistrés dans le fichier export"
}
...
{
regroupementTrois
#ssh $utilisateurSSH@$AdresseIpSSH id $utilisateur | awk '{print $2}' >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt
ssh $utilisateurSSH@$AdresseIpSSH id $utilisateur >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt #a voir avec le groupe celle ci me semble plus plausible
echo "Résultats enregistrés dans le fichier export"
}
...
{
regroupementTrois
ssh $utilisateurSSH@$AdresseIpSSH ls -larth >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt
#ssh $utilisateurSSH@$AdresseIpSSH ls -larth >> $chemin/export/info_$adresseIPSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt a voir avec dom/equipe 
echo "Résultats enregistrés dans le fichier export"
}
...
{
regroupementDeux
ssh $utilisateurSSH@$AdresseIpSSH lsb_release -d >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt
echo "Résultats enregistrés dans le fichier export"
}	
...
{
regroupementDeux
ssh $utilisateurSSH@$AdresseIpSSH df -h >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt	
echo "Résultats enregistrés dans le fichier export"
}	
...
{
regroupementDeux
ssh $utilisateurSSH@$AdresseIpSSH lsblk >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt
echo "Résultats enregistrés dans le fichier export"
}
...
{
regroupementDeux
ssh $utilisateurSSH@$AdresseIpSSH ip a | grep "inet" >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt
}
...
{
regroupementDeux
ssh $utilisateurSSH@$AdresseIpSSH ip a | grep "link/ether" >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt
echo "Résultats enregistrés dans le fichier export"
}
...
{
regroupementDeux
#ssh $utilisateurSSH@$AdresseIpSSH cat /proc/cpuinfo >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt
ssh $utilisateurSSH@$AdresseIpSSH lshw -short | grep "processor" >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt #commande michael je la trouve mieux
echo "Résultats enregistrés dans le fichier export"
}
...
{
regroupementDeux
ssh $utilisateurSSH@$AdresseIpSSH free -h >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt
echo "Résultats enregistrés dans le fichier export"
}
...
{
regroupementDeux
ssh $utilisateurSSH@$AdresseIpSSH ss -tulnp >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt
echo "Résultats enregistrés dans le fichier export"
}
...
{
#regroupementDeux
#ssh $utilisateurSSH@$AdresseIpSSH ufw status >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt
...
{
regroupementDeux
ssh $utilisateurSSH@$AdresseIpSSH cut -d: -f1 /etc/passwd >> $chemin/export/info_$utilisateurSSH\_$(date "+%F")_$(date "+%H:%M:%S").txt
echo "Résultats enregistrés dans le fichier export"
}
